...
export const controller = async () => {
  ...
  const isInstalled = hasInstalledRelatedApps || standalone;
  const installationInfo = document.querySelector('#installation');

  if(installationInfo) {
    installationInfo.style.display = isInstalled ? 'none' : 'block';
  }
  ...
  const expandButton = document.querySelector('#expand-feature-grid');
  ...
  expandButton.addEventListener('click', e => {
    ...
    document.startViewTransition({
      update: () => {
        featureGrid.classList.toggle('expanded');
      },
      types: ['expand-feature-grid']
    });
  });
