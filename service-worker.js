const buildFiles = [
  ...
  'src/img/icons/icon-discord.png',
...
const staticFiles = [
  '_40dannymoerkerke/audio-recorder/dist/audio-recorder.js',
  '_40dannymoerkerke/custom-element/dist/custom-element.es.js',
  '_40dannymoerkerke/material-webcomponents/src/material-app-bar.js',
  '_40dannymoerkerke/material-webcomponents/src/material-bottom-sheet.js',
  '_40dannymoerkerke/material-webcomponents/src/material-button.js',
  '_40dannymoerkerke/material-webcomponents/src/material-checkbox.js',
  '_40dannymoerkerke/material-webcomponents/src/material-dialog.js',
  '_40dannymoerkerke/material-webcomponents/src/material-dropdown.js',
  '_40dannymoerkerke/material-webcomponents/src/material-loader.js',
  '_40dannymoerkerke/material-webcomponents/src/material-progress.js',
  '_40dannymoerkerke/material-webcomponents/src/material-radiobutton-group.js',
  '_40dannymoerkerke/material-webcomponents/src/material-radiobutton.js',
  '_40dannymoerkerke/material-webcomponents/src/material-switch.js',
  '_40dannymoerkerke/material-webcomponents/src/material-textfield.js',
...
const scheduleNotification = async (notification) => {
  try {
    const store = await openStore(IDBConfig.stores.notificationStore, 'readwrite');
...
const retryNotifications = async () => {
  ...
  const responses = await Promise.allSettled(requests);
  const notificationStore = await openStore(IDBConfig.stores.notificationStore, 'readwrite');
  ...
  responses.forEach((response, index) => {
    ...
    if(response.status === 'fulfilled') {
      ...
      console.log(`retrying notification with ${keyPath} ${key} failed: ${response.reason}`);
    }
  });
};
  ...
  e.respondWith(
    (async () => {
      try {
        ...
        const response = await caches.match('offline', {ignoreVary: true, ignoreSearch: true});
        if (response) {
          return response.redirected ? cleanRedirect(response) : response;
        }
      }
...
const messageHandler = async ({data}) => {
  ...
  switch(type) {
    ...
    case 'retry-notifications':
      if(!('sync' in self.registration)) {
        console.log('retry notifications when Background Sync is not supported');
